...
public class Mapper {
	...
	private final ConcurrentMap<String, Set<MappedClass>> mappedClassesByCollection = new ConcurrentHashMap<String, Set<MappedClass>>();
	...
	private MappedClass addMappedClass(MappedClass mc, boolean validate) {
		...
		Set<MappedClass> mcs = Collections.newSetFromMap(new ConcurrentHashMap<MappedClass, Boolean>());
		Set<MappedClass> mcsResult = mappedClassesByCollection.putIfAbsent(mc.getCollectionName(), mcs);
		if (mcsResult != null)
			mcs = mcsResult;
		mcs.add(mc);
	 ...
	 * 
	 * @param entityClass
	 *            The type to return, or use; can be overridden by the @see Mapper.CLASS_NAME_FIELDNAME in the DBObject
	 **/
	...
	public static MappedField validate(Class clazz, Mapper mapr, StringBuffer origProp, FilterOperator op, Object val, boolean validateNames,
		...
		if (validateNames) {
			...
			for (int i = 0;;) {
				...
				if (i < parts.length && !canQueryPast(mf))
					throw new ValidationException("Can not use dot-notation past '" + part + "' could not be found in '" + clazz.getName()
							+ "' while validating - " + prop);
	...
	public static boolean isCompatibleForOperator(Class<?> type, FilterOperator op, Object value) {
		if (value == null || type == null)
			...
			return true;
				...
				&& (value.getClass().isArray() || Iterable.class.isAssignableFrom(value.getClass()) || Map.class.isAssignableFrom(value.getClass())))
			return true;
